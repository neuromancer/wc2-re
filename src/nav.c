 ...
 * OriginFx music playback position, see Wc1SdlServiceOriginFxMusic in
 ...
 * see WC2_CUTSCENE_MOUTH_MIN_TICKS, wc1.h, and opcode 0x7c's own
 ...
 * keeps the authentic (if abbreviated) Win32 cap; WC1_SDL restores the
...
static int g_nNextTitleFrameClock;

static void Wc1SdlPaceTitleFrame(int ticks)
{
    while (g_nInputClock_005c84a8 < g_nNextTitleFrameClock)
        PumpWindowMessages(0);
    g_nNextTitleFrameClock = g_nInputClock_005c84a8 + ticks;
}

#define WC2_TITLE_FRAME_TICKS 6
#define WC2_TITLE_FIREWORKS_TICKS 3
#endif
...
void Title_Sequence(void)
{
    ...
    if (g_nMusicDriverMode_0049be8c == 2 ||
        ...
        while (g_nTitleMusicSequenceStage_0049be94 < 3 &&
               waitCount < 200) {
            Wc1SdlPaceTitleFrame(WC2_TITLE_FRAME_TICKS);
#else
        while (g_nTitleMusicSequenceStage_0049be94 < 3 &&
               waitCount < 3) {
#endif
...
void ShowVictoryScreen(short logoY)
{
    ...
    do {
         ...
         * cap at all -- see the comment above Wc1SdlPaceTitleFrame.
...
#ifdef WC1_SDL
        Wc1SdlPaceTitleFrame(WC2_TITLE_FIREWORKS_TICKS);
#endif
        DrawSpriteDefault(&g_stSecondaryViewBuffer_005d2c90, 0, 0,
                          g_pTitleFieldShape_00491cf8, 0);
        emptyCount = TheEndFireWorks(
