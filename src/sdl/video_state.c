...
{
    if (backend == SDL_VIDEO_BACKEND_GL_SHARP_BILINEAR)
        g_videoBackend = backend;
    else
        g_videoBackend = SDL_VIDEO_BACKEND_INDEXED;
}

int SdlUsingGlRenderer(void)
{
    return g_videoBackend == SDL_VIDEO_BACKEND_GL_SHARP_BILINEAR;
}
                                   ...
                                   int *viewportHeight)
{
    ...
    if ((Sint64)width * SDL_DISPLAY_ASPECT_HEIGHT >
        (Sint64)height * SDL_DISPLAY_ASPECT_WIDTH) {
        *viewportHeight = height;
        *viewportWidth =
            (int)((Sint64)height * SDL_DISPLAY_ASPECT_WIDTH /
                  SDL_DISPLAY_ASPECT_HEIGHT);
    } else {
        *viewportWidth = width;
        *viewportHeight =
            (int)((Sint64)width * SDL_DISPLAY_ASPECT_HEIGHT /
                  SDL_DISPLAY_ASPECT_WIDTH);
    }
    if (*viewportWidth < 1)
        *viewportWidth = 1;
    if (*viewportHeight < 1)
                             ...
                             int *windowX, int *windowY)
{
    ...
    *windowX = viewportLeft +
               SdlScaleCoordinate(logicalX, SDL_FRAME_WIDTH,
                                      viewportWidth);
    *windowY = viewportTop +
               SdlScaleCoordinate(logicalY, SDL_FRAME_HEIGHT,
                                      viewportHeight);
    return 1;
}
